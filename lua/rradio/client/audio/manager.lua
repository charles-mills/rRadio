...
local function shouldMuteForGameMenu()
    return muteGameMenuConVar:GetBool() and gui.IsGameUIVisible()
end
...
local function requestStateRefresh(reason)
    timer.Simple(0, function()
        ...
        if netHandlers and netHandlers.RequestStateSnapshot then
...
local function isEntityMuted(entity)
    if muted[entity] then return true end
...
local function getConfiguredFadeDuration(key, fallback)
    ...
    local duration = rRadio.config.Crossfade[key] or fallback
...
local function getBucket(entity)
    if not sources[entity] then
        sources[entity] = {
            current = nil, currentAssignment = nil,
            incoming = nil, incomingAssignment = nil,
            pendingAssignment = nil, pendingNonce = nil,
            outgoing = {}, outgoingAssignments = {},
            envelopes = {}, channelGains = {}, channel3DEnabled = {}
        }
    end
...
local function removeOutgoing(bucket, channel)
    for i = #bucket.outgoing, 1, -1 do
        if bucket.outgoing[i] == channel then
            table.remove(bucket.outgoing, i)
        end
    end
end
...
local function isBucketEmpty(bucket)
    ...
    if IsValid(bucket.current) or IsValid(bucket.incoming) or bucket.pendingAssignment then return false end
    return not hasValidOutgoing(bucket) and next(bucket.envelopes) == nil
end
...
local function assignmentUsesSameSource(left, right)
    if not left or not right or not left.url or not right.url or left.url == "" or right.url == "" then 
        return false 
    end
...
local function hardStopBucket(entity, clearMute)
    ...
    sources[entity] = nil
    occlusionStates[entity] = nil
    if clearMute then muted[entity] = nil end
end
...
local function getSourcePosition(entity, config)
    ...
    local offset = config and config.SourceOffset
    if isvector(offset) and entity.LocalToWorld then 
        return entity:LocalToWorld(offset) 
    end
    return entity:WorldSpaceCenter() or entity:GetPos()
end
...
local function getFullVolumeDistance(config, maxDistance)
    local full = tonumber(config.FullVolumeDistance) or DEFAULT_FULL_VOLUME_DISTANCE
    return math.Clamp(full, 1, maxDistance)
end
...
local function computeDistanceGain(distance, fullDistance, maxDistance, exponent)
    ...
    return math.Clamp((raw - floor) / denom, 0, 1)
end

local function getOcclusionState(entity)
    if not occlusionStates[entity] then
        occlusionStates[entity] = {
            current = 1, target = 1,
            lastTraceTime = 0, lastUpdateTime = CurTime()
        }
    end
    return occlusionStates[entity]
end

local function smoothOcclusionState(state, now, config)
    local elapsed = math.max(now - (state.lastUpdateTime or now), 0)
    state.lastUpdateTime = now
    local speed = math.max(tonumber(config.SmoothingSpeed) or DEFAULT_OCCLUSION_SMOOTHING_SPEED, 0)
    if speed <= 0 then
        state.current = state.target or 1
    else
        local alpha = 1 - math.exp(-elapsed * speed)
...
local function computeOcclusionGain(player, entity, sourcePos, distanceGain)
    ...
    if config.Enabled == false or distanceGain <= 0 then return 1 end
    local state = getOcclusionState(entity)
    local now = CurTime()
    local interval = math.max(tonumber(config.TraceInterval) or DEFAULT_OCCLUSION_TRACE_INTERVAL, 0)
    if now - (state.lastTraceTime or 0) >= interval and occlusionTraceBudget > 0 then
...
local function beginOcclusionTraceBatch()
    local config = rRadio.config.Occlusion
    occlusionTraceBudget = (config.Enabled == false) and 0 or 
        math.max(math.floor(tonumber(config.MaxTracesPerTick) or DEFAULT_OCCLUSION_TRACE_BUDGET), 0)
end
...
local function buildEntityAudioContext(entity, player, context)
    context = context or {config = getEntityConfig(entity)}
    if not context.config then return nil end
    context.sourcePos = context.sourcePos or getSourcePosition(entity, context.config)
    ...
    if not enabledConVar:GetBool() or isEntityMuted(entity) or shouldMuteForGameMenu() then 
        return context 
    end
    ...
    local dist = math.sqrt(player:GetPos():DistToSqr(context.sourcePos))
    local maxDist = getMaxHearingDistance(context.config)
    local fullDist = getFullVolumeDistance(context.config, maxDist)

    context.distanceGain = computeDistanceGain(dist, fullDist, maxDist, context.config.DistanceFalloffExponent)
    context.occlusionGain = computeOcclusionGain(player, entity, context.sourcePos, context.distanceGain)
    return context
end

local function isWithinLoadRange(player, entity)
    local config = getEntityConfig(entity)
    local sourcePos = getSourcePosition(entity, config)
    local range = getMaxHearingDistance(config) * (rRadio.config.LoadDistanceFactor or 2)
    return player:GetPos():DistToSqr(sourcePos) <= range * range
end

local function isBeyondUnloadRange(player, entity)
    local config = getEntityConfig(entity)
    local sourcePos = getSourcePosition(entity, config)
...
local function computeBaseVolume(context, volume)
    ...
    return clampLocalVolume(volume) * (context.distanceGain or 0) * (context.occlusionGain or 1)
end
...
local function getEnvelopeProgress(envelope)
    ...
    return math.Clamp((SysTime() - envelope.startTime) * 1000 / durationMs, 0, 1)
end
...
local function getEnvelopeGain(bucket, channel)
    ...
    return math.Clamp(envelope.startGain + (envelope.targetGain - envelope.startGain) * progress, 0, 1)
end
...
updatePresentationFromChannel = function(entity, bucket, channel, baseVolume, effectiveVolume)
    ...
    elseif channel == bucket.incoming and effectiveVolume > AUDIBLE_GAIN_THRESHOLD then
        ...
        local isForeground = competingGain <= AUDIBLE_GAIN_THRESHOLD or 
                            effectiveVolume >= competingGain or 
                            envelopeGain >= CROSSFADE_FOREGROUND_ENVELOPE_GAIN
...
local function collectBucketChannels(bucket, channels)
    ...
    if IsValid(bucket.current) then channels[#channels+1] = bucket.current end
    if IsValid(bucket.incoming) then channels[#channels+1] = bucket.incoming end
    for _, ch in ipairs(bucket.outgoing) do
        if IsValid(ch) then channels[#channels+1] = ch end
    end
    return channels
end
...
local function fadeChannelOut(entity, bucket, channel, durationMs, curve)
    ...
    if not hasValidOutgoing(bucket) or not table.HasValue(bucket.outgoing, channel) then
...
local function promoteIncoming(entity, bucket, channel)
    if sources[entity] ~= bucket or bucket.incoming ~= channel or not IsValid(channel) then return end
    ...
    if not bucket.pendingAssignment and bucket.currentAssignment then
        ...
        setPresentationPhase(entity, current and current.phase or phases.CONNECTING,
            bucket.currentAssignment.stationName, bucket.currentAssignment.stationID)
    end
...
local function capOutgoing(entity, bucket)
    local maxOut = getMaxOutgoing()
    while #bucket.outgoing > maxOut do
...
local function clearPendingPlayback(entity, bucket)
    if not bucket or not bucket.pendingAssignment then return end
    bumpNonce(entity)
    bucket.pendingAssignment = nil
    bucket.pendingNonce = nil
end
...
local function updateMatchingAssignment(entity, bucket, assignment)
    ...
    if not (matchesCurrent or matchesIncoming or matchesPending or matchesQueued) then
        return false
    end
    ...
    if matchesCurrent then
        ...
        local current = presentationStates[entity]
        setPresentationPhase(entity, current and current.phase or phases.CONNECTING,
            assignment.stationName, assignment.stationID)
    ...
    sound.PlayURL(assignment.url, "3d noplay", function(source)
        ...
        if not IsValid(source) or not IsValid(entity) then
            if IsValid(entity) then
                local b = sources[entity]
                if b then b.pendingAssignment = nil; b.pendingNonce = nil end
                fadeAudibleChannelsOut(entity, b, getStopFadeOutDuration(), "linear")
                retryPlayback(assignment, entity, nonce)
            end
            return
        end
        ...
        local hadAudiblePlayback = IsValid(previousCurrent) or IsValid(previousIncoming) or hasValidOutgoing(bucket)
...
function manager.StopAll()
    local entities = {}
    for entity in pairs(sources) do entities[entity] = true end
    for entity in pairs(queued) do entities[entity] = true end
    for entity in pairs(activeAssignments) do entities[entity] = true end
...
local function appendConnectedRow(rows, entity, bucket, channel, role)
    if not IsValid(channel) then return end
    local assignment = getChannelAssignment(bucket, channel)
    local volume = volumes[entity] or (assignment and assignment.volume) or 0
    rows[#rows + 1] = {
        entity = entity,
        role = role,
...
local function updateSources(player)
    ...
    for entity, bucket in pairs(sources) do
        if not IsValid(entity) then
            ...
            if rRadio.config.ConditionalStationUnload and isBeyondUnloadRange(player, entity) then
                local assignment = activeAssignments[entity]
                bumpNonce(entity)
                hardStopBucket(entity)
                if assignment then
                ...
                local context = buildEntityAudioContext(entity, player)
...
local function updateEnvelopes()
    for entity, bucket in pairs(sources) do
        if not IsValid(entity) then
            clearPlaybackState(entity, true)
        elseif next(bucket.envelopes) then
...
local function addDebugCommands()
    ...
    end, nil, rRadio.L("CommandDisconnectAllHelp", "Disconnect all local rRadio streams."), FCVAR_CLIENTCMD_CAN_EXECUTE)
    ...
    concommand.Add("rammel_rradio_list_active", function()
        ...
        if #known == 0 then
            ...
            for _, row in ipairs(known) do
                print("[rRadio]", " ", row.entity, row.queued and "queued" or tostring(row.phase), row.stationID, row.stationName, row.volume)
            end
        end
        ...
        for _, row in ipairs(connected) do
            print("[rRadio]", " ", row.entity, row.role, row.stationID, row.stationName, row.volume, row.url)
        end
    end, nil, rRadio.L("CommandListActiveHelp", "List known and connected local rRadio streams."), FCVAR_CLIENTCMD_CAN_EXECUTE)
end
...
function manager.Init()
    ...
    cvars.AddChangeCallback("rammel_rradio_enabled", function(_, _, newValue)
