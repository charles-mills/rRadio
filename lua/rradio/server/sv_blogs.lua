if SERVER and GAS then CreateConVar("rammel_rradio_logging", "0", FCVAR_ARCHIVE, "Enable bLogs integration for rRadio") end
...
MODULE_PLAYS:Setup(function()
    local function StationPlayed(ply, ent, station, stationURL)
        ...
        MODULE_PLAYS:LogPhrase("Station Played by " .. (ply and GAS.Logging:FormatPlayer(ply) or "Unknown"), {{"Player", GAS.Logging:FormatPlayer(ply)}, {"Entity", GAS.Logging:FormatEntity(ent)}, {"Station", GAS.Logging:Escape(station)}, {"Station URL", GAS.Logging:Escape(stationURL)}})
    end
...
MODULE_STOPS:Setup(function()
    local function StationStopped(ply, ent)
        if not loggingEnabled then return end
        MODULE_STOPS:LogPhrase("Station Stopped by " .. (ply and GAS.Logging:FormatPlayer(ply) or "Unknown"), {{"Player", GAS.Logging:FormatPlayer(ply)}, {"Entity", GAS.Logging:FormatEntity(ent)},})
    end
...
cvars.AddChangeCallback("rammel_rradio_logging", function(convar_name, old_value, new_value)
    if tobool(new_value) then
        ...
        rRadio.logger.InfoScope("blogs", "Disabled rRadio bLogs integration.")
    end
