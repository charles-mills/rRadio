...
function rRadio.utils.GetVehicle(ent)
    ...
    if rRadio.utils.VehicleClasses[ent:GetClass()] or ent:IsVehicle() or
       ent:GetClass():match("^lvs_") or ent:GetClass():match("^ses_") or
       ent:GetClass():match("^sw_") or ent:GetClass():match("^drs_") then
        return ent
    end
end
...
function rRadio.utils.isSitAnywhereSeat(vehicle)
    ...
    if rRadio.utils.SitAnywhereSeats[vehicle:GetClass()] then return true end
...
function rRadio.utils.canInteractWithBoombox(ply, boombox)
    ...
    rRadio.DevPrint("Checking boombox interaction for " .. ply:Nick())
    local owner = rRadio.sv.utils and rRadio.sv.utils.getOwner(boombox)
    if owner == ply then
        rRadio.DevPrint("Player is owner")
        return true
    end
    if CAMI.PlayerHasAccess(ply, "rradio.UseAll", nil) then
...
function rRadio.utils.GetEntityConfig(entity)
    ...
    if entityClass == "rammel_boombox_gold" then return rRadio.config.GoldenBoombox
    elseif entityClass == "rammel_boombox" then return rRadio.config.Boombox
    elseif rRadio.utils.GetVehicle(entity) then return rRadio.config.VehicleRadio
    end
end
...
function rRadio.utils.setRadioStatus(entity, status, stationName, isPlaying, updateNameOnly)
    ...
    if timer.Exists("UpdateBoomboxStatus_" .. entIndex) then timer.Remove("UpdateBoomboxStatus_" .. entIndex) end
    stationName = stationName or ""
    isPlaying = isPlaying == nil and (status == "playing" or status == "tuning") or isPlaying
    local statuses = SERVER and rRadio.sv.BoomboxStatuses or rRadio.cl.BoomboxStatuses or {}
    statuses[entIndex] = statuses[entIndex] or {}
    ...
    if SERVER then
        ...
        net.WriteString(updateNameOnly and statuses[entIndex].stationStatus or status or "")
        net.Broadcast()
    end
end
...
function rRadio.utils.canUseRadio(entity)
    ...
    if not vehicle or rRadio.utils.isSitAnywhereSeat(vehicle) then return false end
    return true
end

function rRadio.utils.PrintVehicleClassInfo(ent)
    if not IsValid(ent) then
        rRadio.DevPrint("Invalid entity passed to PrintVehicleClassInfo")
        return
    end
    rRadio.DevPrint("Entity Class: " .. ent:GetClass())
    local parent = ent:GetParent()
    if IsValid(parent) then
        rRadio.DevPrint("Parent Class: " .. parent:GetClass())
    else
        rRadio.DevPrint("Entity has no valid parent")
    end
end

function rRadio.utils.FormatAndTranslateCountry(rawKey)
    local formatted = rawKey:gsub("_", " "):gsub("(%a)([%w_']*)", function(f, r) return f:upper() .. r:lower() end)
...
if CLIENT then
    rRadio.keyCodeMapping = {
        ...
        [KEY_9] = "9", [KEY_PAD_0] = "Numpad 0", [KEY_PAD_1] = "Numpad 1",
        [KEY_PAD_2] = "Numpad 2", [KEY_PAD_3] = "Numpad 3", [KEY_PAD_4] = "Numpad 4",
        [KEY_PAD_5] = "Numpad 5", [KEY_PAD_6] = "Numpad 6", [KEY_PAD_7] = "Numpad 7",
        [KEY_PAD_8] = "Numpad 8", [KEY_PAD_9] = "Numpad 9", [KEY_PAD_DIVIDE] = "Numpad /",
        [KEY_PAD_MULTIPLY] = "Numpad *", [KEY_PAD_MINUS] = "Numpad -", [KEY_PAD_PLUS] = "Numpad +",
        [KEY_PAD_ENTER] = "Numpad Enter", [KEY_PAD_DECIMAL] = "Numpad .",
        [KEY_LSHIFT] = "Left Shift", [KEY_RSHIFT] = "Right Shift", [KEY_LALT] = "Left Alt",
        [KEY_RALT] = "Right Alt", [KEY_LCONTROL] = "Left Ctrl", [KEY_RCONTROL] = "Right Ctrl",
        ...
        [KEY_F11] = "F11", [KEY_F12] = "F12", [KEY_CAPSLOCKTOGGLE] = "Caps Lock Toggle",
        [KEY_NUMLOCKTOGGLE] = "Num Lock Toggle", [KEY_LAST] = "Last Key"
    }

    function rRadio.utils.GetKeyName(keyCode)
        return rRadio.keyCodeMapping[keyCode] or "Unknown Key"
    end
end
