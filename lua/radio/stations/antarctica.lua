local stations = {
    {name = "A Net Station", url = "http://anetstation.com/MP3/ANETSTATIONDOTCOM.m3u"},
    {name = "A Net Station", url = "https://anetstation.com/MP3/ANETSTATIONDOTCOM.m3u"},
    {name = "Doujindance", url = "http://radio.funkot.net:8010/playlist"},
    {name = "kbs", url = "http://serpent0.duckdns.org:8088/kbs1fm.pls"},
    {name = "lassulus radio", url = "http://radio.lassul.us/radio.mp3"},
    {name = "Radio Cirqle", url = "http://82.209.210.183:8000/aac"},
    {name = "UKNova", url = "http://uknr.org:8000/"},
}

return stations
