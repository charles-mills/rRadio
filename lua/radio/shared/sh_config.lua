...
local carRadioShowMessages =
    EnsureConVar("car_radio_show_messages", "1", true, "Enable or disable car radio animations on entering a vehicle.")
local maxVolumeCvar =
    CreateServerConVar("radio_max_volume_limit", "1.0", "Maximum volume limit for all radio entities (0.0-1.0)")
local messageCooldownCvar =
    CreateServerConVar(
...
local boomboxMaxDistCvar =
    CreateServerConVar("radio_boombox_max_distance", "800", "Maximum hearing distance for boomboxes")
local boomboxMinDistCvar =
    CreateServerConVar("radio_boombox_min_distance", "500", "Distance at which boombox volume starts to drop off")
local goldenVolumeCvar = CreateServerConVar("radio_golden_boombox_volume", "1.0", "Default volume for golden boomboxes")
local goldenMaxDistCvar =
    CreateServerConVar("radio_golden_boombox_max_distance", "350000", "Maximum hearing distance for golden boomboxes")
local goldenMinDistCvar =
    CreateServerConVar(
    "radio_golden_boombox_min_distance",
    "250000",
    "Distance at which golden boombox volume starts to drop off"
)
local vehicleVolumeCvar = CreateServerConVar("radio_vehicle_volume", "1.0", "Default volume for vehicle radios")
local vehicleMaxDistCvar =
    CreateServerConVar("radio_vehicle_max_distance", "800", "Maximum hearing distance for vehicle radios")
local vehicleMinDistCvar =
    CreateServerConVar("radio_vehicle_min_distance", "500", "Distance at which vehicle radio volume starts to drop off")
Config.Boombox = {
    Volume = function()
        return boomboxVolumeCvar:GetFloat()
    end,
    MaxHearingDistance = function()
        return boomboxMaxDistCvar:GetFloat()
    end,
    MinVolumeDistance = function()
        return boomboxMinDistCvar:GetFloat()
    end,
    RetryAttempts = 3,
    RetryDelay = 2
}
Config.GoldenBoombox = {
    Volume = function()
        return goldenVolumeCvar:GetFloat()
    end,
    MaxHearingDistance = function()
        return goldenMaxDistCvar:GetFloat()
    end,
    MinVolumeDistance = function()
        return goldenMinDistCvar:GetFloat()
    end,
    RetryAttempts = 3,
    RetryDelay = 2
}
Config.VehicleRadio = {
    Volume = function()
        return vehicleVolumeCvar:GetFloat()
    end,
    MaxHearingDistance = function()
        return vehicleMaxDistCvar:GetFloat()
    end,
    MinVolumeDistance = function()
        return vehicleMinDistCvar:GetFloat()
    end,
    RetryAttempts = 3,
    RetryDelay = 2
}
Config.MessageCooldown = function()
    return messageCooldownCvar:GetFloat()
end
Config.MaxVolume = function()
    return maxVolumeCvar:GetFloat()
end
...
local function loadStationsForCountry(rawCountryName)
    ...
    if file.Exists(path, "LUA") then
        ...
        if stations then
            ...
            print(string.format("[rRADIO] Failed to include stations from %s", path))
        end
    else
        print(string.format("[rRADIO] Station file does not exist: %s", path))
    end
end
    ...
    for name, defaultValue in pairs(Config.RegisteredConVars.server) do
        ...
        if cvar then
            ...
            if name == "radio_boombox_volume" then
                Config.Boombox.Volume = function()
                    return value
                end
            elseif name == "radio_boombox_max_distance" then
                Config.Boombox.MaxHearingDistance = function()
                    return value
                end
            elseif name == "radio_boombox_min_distance" then
                Config.Boombox.MinVolumeDistance = function()
                    return value
                end
            elseif name == "radio_golden_boombox_volume" then
                Config.GoldenBoombox.Volume = function()
                    return value
                end
            elseif name == "radio_golden_boombox_max_distance" then
                Config.GoldenBoombox.MaxHearingDistance = function()
                    return value
                end
            elseif name == "radio_golden_boombox_min_distance" then
                Config.GoldenBoombox.MinVolumeDistance = function()
                    return value
                end
            elseif name == "radio_vehicle_volume" then
                Config.VehicleRadio.Volume = function()
                    return value
                end
            elseif name == "radio_vehicle_max_distance" then
                Config.VehicleRadio.MaxHearingDistance = function()
                    return value
                end
            elseif name == "radio_vehicle_min_distance" then
                Config.VehicleRadio.MinVolumeDistance = function()
                    return value
                end
...
function Config.CalculateVolume(entity, player, distanceSqr)
    if not IsValid(entity) or not IsValid(player) then
        return 0
    end
    ...
    local falloff =
        1 -
        math.Clamp((distance - entityConfig.MinVolumeDistance()) / (maxDist - entityConfig.MinVolumeDistance()), 0, 1)
