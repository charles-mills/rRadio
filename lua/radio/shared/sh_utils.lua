...
--[[
    Function: isInStationRange
    Description: Checks if a player is within hearing range of a radio entity
    @param player (Player): The player to check
    @param entity (Entity): The radio entity
    @return (boolean): True if player is in range
]]
function utils.isInStationRange(player, entity)
    if not IsValid(player) or not IsValid(entity) then return false end
    
    local entityConfig = utils.GetEntityConfig(entity)
    if not entityConfig then return false end
    
    local maxDistance = entityConfig.MaxHearingDistance()
    local playerPos = player:GetPos()
    local entityPos = entity:GetPos()
    
    -- Players in vehicles always hear their own radio
    if utils.isPlayerInVehicle(player, entity) then
        return true
    end
    
    return playerPos:DistToSqr(entityPos) <= (maxDistance * maxDistance)
end

--[[
    Function: getPlayersInRange
    Description: Gets all players within hearing range of an entity
    @param entity (Entity): The radio entity
    @return (table): Table of players in range
]]
function utils.getPlayersInRange(entity)
    if not IsValid(entity) then return {} end
    ...
    for _, player in ipairs(player.GetAll()) do
        if utils.isInStationRange(player, entity) then
