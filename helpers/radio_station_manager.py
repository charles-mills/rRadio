...
def save_stations_to_file(country: str, stations: List[Dict[str, str]], backup_dir: Optional[str] = None) -> None:
    ...
    # Backup the existing file if it exists and backup_dir is provided
...
async def fetch_and_save_stations_concurrently(auto_run: bool = False):
...
async def fetch_and_save_country_stations(session: aiohttp.ClientSession, country: str, semaphore: asyncio.Semaphore, backup_dir: Optional[str], pbar: tqdm) -> None:
...
def main_menu(auto_run: bool = False) -> None:
    if auto_run:
        ...
        asyncio.run(fetch_and_save_stations_concurrently(auto_run=True))
        asyncio.run(verify_all_stations())
        asyncio.run(count_and_push_station_count())
        logging.info("Auto-run completed.")
    else:
        while True:
