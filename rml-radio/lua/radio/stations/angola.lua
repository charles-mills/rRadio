local stations = {
    {name = "EXA FM: Música Pop Em Inglês E Espanhol", url = "https://playerservices.streamtheworld.com/api/livestream-redirect/XHPSFMAAC.aac"},
    {name = "ANGOLA FM", url = "https://canli.arabeskinmerkezi.com/9180/stream"},
    {name = "Absolute Country", url = "http://strm112.1.fm/acountry_mobile_mp3"},
    {name = "Dimusic Club Angola 🇦🇴", url = "http://stream1.dimusic.club:11496/live?type=.aac"},
    {name = "Ebnézer Rádio", url = "http://08.stmip.net:10028/;;"},
    {name = "LAC Luanda Antena Comercial 955 Mhz FM, Luanda", url = "http://radios.vpn.sapo.pt/AO/radio14.mp3"},
    {name = "Radio Escola 885 FM", url = "http://radios.vpn.sapo.pt/AO/radio1.mp3"},
    {name = "Rádio Fé Angola", url = "http://stream.zeno.fm/gsndh1xvczetv"},
    {name = "Radio Mais 991", url = "http://radios.vpn.sapo.pt/AO/radio10.mp3"},
    {name = "Rádio MFM Angola", url = "https://centova87.instainternet.com/proxy/mfm?mp=/stream"},
    {name = "Radio N'Gola Yetu", url = "https://radios.vpn.sapo.pt/AO/radio12.mp3"},
    {name = "Radio Romantica", url = "https://radios.vpn.sapo.pt/AO/radio4.mp3"},
    {name = "Radio Sem Anestesia", url = "https://radios.vpn.sapo.pt/AO/radio7.mp3"},
    {name = "Rádio Unia 923 Mhz FM, Luanda Universidade Independente De Angola", url = "http://radios.vpn.sapo.pt/AO/radio2.mp3"},
    {name = "RNA Rádio 5 945 Mhz FM, Luanda Rádio Nacional De Angola", url = "http://radios.vpn.sapo.pt/AO/radio5.mp3"},
}

return stations
